...
export class GameGateway implements OnGatewayDisconnect {
  ...
  async handleAttack(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    data: { roomId: string; targetId: string; riskCardId: string },
  ) {
    try {
      const playerId = this.clientToPlayer.get(client.id) || client.id;
      const room = await this.gameService.attack(
        data.roomId,
        playerId,
        data.targetId,
        data.riskCardId,
  ...
  async handleDefend(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    data: { roomId: string; success: boolean; mitigationCardId?: string },
  ) {
    try {
      const playerId = this.clientToPlayer.get(client.id) || client.id;
      const room = await this.gameService.defend(
        data.roomId,
        playerId,
        data.success,
        data.mitigationCardId,
      );
